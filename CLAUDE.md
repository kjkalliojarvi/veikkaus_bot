...
- `database.py` — the working one. Raw `sqlite3` with a `db_ops` context manager and `Db` class. Tables: `race`, `runner`, `start`, `stat`, `bet_percentage`. `store_file()` loads a saved JSON dump; `store_data()` consumes a `VeikkausData` directly. Table column order matches the `to_json()` tuples.
